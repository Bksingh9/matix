...
/* `licence` fails closed; everything else fails open. A limiter outage must
   not take the product down, but the licence route is the only one where
   guessing a value yields a paid product, so there an outage means "refuse"
   rather than "allow unlimited attempts". */
const FAIL_CLOSED = new Set(['licence']);
...
export async function allow(bucket, limit, windowSeconds) {
  try {
    ...
    const kind = String(bucket).split(':')[0];
    if (FAIL_CLOSED.has(kind)) {
      console.error(`[ratelimit] backend unavailable, REFUSING ${kind}:`, e.message);
      return false;
    }
